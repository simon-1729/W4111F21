...
class CSVTable:
    ...
    def __init__(self, t_name, load=True):
        ...
        if load:
            ...
            self.__rows__ = []  # a list of dictionaries
    ...
    def __add_row__(self, row):
        ...
        for _,index in defined_indexes.items():
    ...
    def __load__(self):
        ...
        given_indexes = list(self.__description__.indexes.values())
        self.__keys_added__ = []
    ...
    def __get_access_path__(self, fields):
        ...
        try:
            ...
            indexes = None
        if (indexes is None or indexes == {}) or fields is None:
            return None, None
        count = 0
        res_index = None
        for index_name, index_rows in indexes.items():
            index_col_names = self.__description__.indexes[index_name].column_names
            applicable = True
            for index_col_name in index_col_names:
                if index_col_name not in fields:
                    applicable = False
                    break
            if applicable:
                if len(self.__indexes__[index_name]) > count:
                    res_index = index_name
                    count = len(self.__indexes__[index_name])
        if count == 0:
            count = None
        return res_index, count
    ...
    def project(self, rows, fields):
        ...
        try:
            if fields is None:  # If there is not project clause, return the base table
                ...
                return result  # a list of dictionaries
    ...
    def __find_by_template_scan__(self, t, fields=None):
        ...
        for row in self.__rows__:
            if self.matches_template(row, t):
                res_rows.append(row)
        return self.project(res_rows, fields) # a list of dictionaries
    def __find_by_template_index__(self, t, idx_name, fields=None):
        ...
        """
        # ************************ TO DO ***************************
        key_string = self.__get_key__(self.__description__.indexes[idx_name], t)
        res_rows = []
        for row in self.__indexes__[idx_name][key_string]:
            if self.matches_template(row, t):
                res_rows.append(row)
        return self.project(res_rows, fields)
    ...
    def __smart_join__(self, right_r, on_fields, where_template=None, project_fields=None):
        ...
        # Optimization 1
        # Compute the new left and right tables using the where_template if it is not None
        left_r = self
        left_rows = left_r.__get_row_list__()
        right_rows = right_r.__get_row_list__()
        result_rows = []
        left_table_name = left_r.__table_name__
        right_table_name = right_r.__table_name__
        if where_template is not None:
            left_rows = left_r.__find_by_template__(left_r.__get_sub_where_template__(where_template))
            right_rows = right_r.__find_by_template__(right_r.__get_sub_where_template__(where_template))

        # Optimization 2
        # Compute the counts of best possible index of the on_fileds using __get_access_path__()
        # for left and right tables.
        # If neither of the indexes is None
        # Compare the number_of_rows/count of left and right tables, and choose the smaller one as the "right table"
        # If index of a table is not None and the other one's index is None
        # then choose the table with not None index as the "right table"
        # If the indexes are both None, the same as dumb join
        index_1, count1 = left_r.__get_access_path__(on_fields)
        index_2, count2 = right_r.__get_access_path__(on_fields)
        opt2 = True
        if index_1 is None and index_2 is None:
            opt2 = False
        if index_1 is not None:
            if index_2 is None or len(left_r.__get_row_list__())/count1 < len(right_r.__get_row_list__())/count2:
                tmp_rows = right_rows
        ...
        for lr in left_rows:
            on_template = self.__get_on_template__(lr, on_fields)
            if opt2:
                rows_of_index = right_r.__find_by_template__(on_template)  # will use find_by_template_index()
                sub_where_tmpl = right_r.__get_sub_where_template__(where_template)
                for rr in rows_of_index:
                    if self.matches_template(rr, on_template) \
                            and self.matches_template(rr, sub_where_tmpl):
                        new_r = {**lr, **rr}
                        result_rows.append(new_r)
            else:
                for rr in right_rows:
